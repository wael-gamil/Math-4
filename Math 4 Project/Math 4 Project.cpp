...
public:
    ...
    string EditEquation(string eq)
    {
        ...
        for (int i = 0; i < eq.length(); i++)
        {
            ...
            else if (eq[i] == '+' || eq[i] == '-' || eq[i] == '*' || eq[i] == '/' || eq[i] == '(' || eq[i] == ')' || eq[i] == '^')
            {
                op == "" ? res += eq[i] : res += OperatorAfterChange(op) + eq[i];
                op = "";
            }
            else if ((eq[i] == 'x' || eq[i] == 'y') && (i == 0 || eq[i - 1] != 'e'))
            {
                op == "" ? res += eq[i] : res += OperatorAfterChange(op) + eq[i];
                op = "";
            }
...
public:
    ...
    bool NewtonSolve(string fx, string fxD1, double root)
    { /// x of (n+1) = x of (n) * ( ss1 / ss2 );
        ...
        if (table[0] != 'y' && table[0] != 'Y')
            cout << fixed << setprecision(9) << "x = " << xiA << endl
            << "fx = " << valueOfFx << endl;
        return notWrong;
    }
...
public:
    ...
    double Central(double x, double y)
    { // 2 x=i+1 y=i-1
        double bak = 0;
        bak = (x - y) / 2 * hval;
        return bak;
    ...
    double backwardsec(double x, double y, double a)
    { // 4 y=i x=i-1 a=i-2
        double bak = 0;
        bak = (y - 2 * x + a) / pow(hval, 2);
        return bak;
    }
    double centralsec(double x, double y, double z)
    { // 5 x=i+1 y=i z=i-1
        double bak = 0;
    ...
    double forwardsec(double x, double y, double a)
    { // 6 a=i+2 x=i+1 y=i
        double bak = 0;
